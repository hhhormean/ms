...
<body>
    ...
    <div>
        <a href="<?php echo U('retrieve');?>" class="btn">所有直播</a>
        <a href="<?php echo U('retrieve?status=1');?>" class="btn">直播中</a>
        <a href="<?php echo U('retrieve?status=0');?>" class="btn">未开始</a>
        <a href="<?php echo U('retrieve?status=3');?>" class="btn">已结束</a>
    </div>
        ...
        <div class="data-table table-striped">
            <table>
                ...
                <?php if(!empty($_list)): if(is_array($_list)): $i = 0; $__LIST__ = $_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                            ...
                            <td><?php echo ($vo["activityId"]); ?></td>
                            <td>
                                <?php switch($vo['activityStatus']): case "0": ?>未开始<?php break;?>
                                    <?php case "1": ?>直播中<?php break;?>
                                    <?php case "2": ?>已中断<?php break;?>
                                    <?php case "3": ?>已结束<?php break;?>
                                    <?php default: ?>未知<?php endswitch;?>
                            </td>
                            <td><?php echo date('Y-m-d H:i', strtotime($vo['startTime']));?></td>
                            <td><?php echo date('Y-m-d H:i', strtotime($vo['endTime']));?></td>
                            <td><?php echo strlen($vo['description']) <= 18 ? $vo['description'] : substr_replace($vo['description'],' .... ',18);?>
                            </td>
                            <td>
                                <a href="<?php echo U('detail?activityId='.$vo['activityId']);?>">详情</a>

                                <?php if($vo['activityStatus'] == 1 || $vo['activityStatus'] == 0): ?><a href="<?php echo U('update?activityId='.$vo['activityId']);?>">修改</a><?php endif; ?>
                                <?php if($vo['activityStatus'] == 1): ?><a href="<?php echo U('get_play_url?activityId='.$vo['activityId']);?>">播放</a><?php endif; ?>
                            </td>
